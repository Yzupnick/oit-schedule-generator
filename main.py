...
def step_to_events(step: Step, start: datetime.datetime):
    ...
    for day_num in range(step.intended_length):
        ...
        event.make_all_day()
        events.append(event)
    for delay_day_num in range(1, step.delay_length + 1):
        event = Event()
        ...
        event.make_all_day()
        events.append(event)
    return ToCalendar(events=events, end=start + datetime.timedelta(days=step.intended_length + step.delay_length))
...
def transition_to_events(transition, start: datetime.datetime, from_, to ):
    ...
    while current_ml <= transition.ml_end_number:
        event = Event()
        event.name = f"Dose: {current_ml}ml from 3oz mixture"
        event.description = (
            f"Transition Day {current_day}/{total_days} (from Step {from_.name} to {to.name})\n"
    ...
    return ToCalendar(events=events, end=start + datetime.timedelta(days=current_day))
...
def create_medication_schedule():
    ...
    medication_steps = [
        ...
        Step(id=2,name="2 small scoops", intended_length=14),
        Transition(from_=2, to=3),
    ...
    while True:
        ...
        try:
            ...
            initial_start_date = current_time
            break
